...
int main()
{
	...
	std::array<vk::AttachmentDescription, 2> attachmentDescriptions{
		...
		.setLoadOp(vk::AttachmentLoadOp::eLoad)		// What do do with the image when the renderpass starts?
		.setStoreOp(vk::AttachmentStoreOp::eStore)	// What to do with the image when the renderpass has finished?
		.setInitialLayout(vk::ImageLayout::eTransferDstOptimal)	// When the renderpass starts, in which layout will the image be?
		.setFinalLayout(vk::ImageLayout::ePresentSrcKHR),	// When the renderpass finishes, in which layout shall the image be transfered?
		...
		.setLoadOp(vk::AttachmentLoadOp::eClear)		// What do do with the image when the renderpass starts?
		...
		.setFinalLayout(vk::ImageLayout::eDepthStencilAttachmentOptimal),	// When the renderpass finishes, in which layout shall the image be transferred?
	};
	...
	std::array<vk::SubpassDependency, 2> subpassDependencies{
		...
		.setSrcStageMask(vk::PipelineStageFlagBits::eTransfer) /* -> */ .setDstStageMask(vk::PipelineStageFlagBits::eColorAttachmentOutput)
						   .setSrcAccessMask(vk::AccessFlags{vk::AccessFlagBits::eTransferWrite}) /* -> */ .setDstAccessMask(vk::AccessFlags{vk::AccessFlagBits::eColorAttachmentWrite}),
	vk::SubpassDependency{}
		...
		.setSrcStageMask(vk::PipelineStageFlagBits::eColorAttachmentOutput) /* -> */ .setDstStageMask(vk::PipelineStageFlagBits::eBottomOfPipe)
						   .setSrcAccessMask(vk::AccessFlags{vk::AccessFlagBits::eColorAttachmentWrite}) /* -> */ .setDstAccessMask(vk::AccessFlags{})
	};
	...
	for (size_t i = 0; i < CONCURRENT_FRAMES; ++i) {
		...
		commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipelineLayout, 0u, 1u, &descriptorSets[0], 0u, nullptr);
