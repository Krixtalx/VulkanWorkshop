...
int main()
{
	...
	std::array<vk::Fence, CONCURRENT_FRAMES> syncFence;
	for (size_t i = 0; i < CONCURRENT_FRAMES; ++i) {
		...
		syncFence[i] = device.createFence(vk::FenceCreateInfo{}.setFlags(vk::FenceCreateFlagBits::eSignaled));
	}

	// ===> 13. Start our render loop and display a wonderful sprite animation:
	const double startTime = glfwGetTime();
	auto frameIndex = int64_t{ -1 };
	while (!glfwWindowShouldClose(window)) {
		...
		// ------------------------------------------------------------------------------
		frameIndex = (frameIndex + 1) % CONCURRENT_FRAMES;
		auto swapChainImageIndex = device.acquireNextImageKHR(swapchain, std::numeric_limits<uint64_t>::max(), imageAvailableSemaphores[frameIndex], nullptr).value;
		auto& currentSwapchainImage = swapchainImages[swapChainImageIndex];
		device.waitForFences(syncFence[frameIndex], VK_TRUE, UINT64_MAX);
		device.resetFences(syncFence[frameIndex]);


		// Submit the command buffer
		auto selectedCommandBuffer = preparedCommandBuffers[frameIndex][explosionAniIndex];
		// ------------------------------------------------------------------------------
		...
		auto submitInfo = vk::SubmitInfo{}
			...
			.setPWaitSemaphores(&imageAvailableSemaphores[frameIndex]) // <--- (*1) Wait on the swap chain image to become available
			.setSignalSemaphoreCount(1u)
			.setPSignalSemaphores(&renderFinishedSemaphores[frameIndex]) // Another semaphore: This will be signalled as soon as this batch of work has completed. 
			.setPWaitDstStageMask(&waitStage);
		queue.submit({ submitInfo }, syncFence[frameIndex]); // <--- Hint: the second parameter is yearning for a fence... ;) ;) 
		...
		auto presentInfo = vk::PresentInfoKHR{}
			...
			.setWaitSemaphoreCount(1u)
			.setPWaitSemaphores(&renderFinishedSemaphores[frameIndex]); // Wait until rendering has finished (until vkQueueSubmit has signalled the renderFinishedSemaphore)
		...
		// TODO Part 3: Remove the waitIdle call and deal with the consequences!
	...
	for (size_t i = 0; i < CONCURRENT_FRAMES; ++i) {
		...
		device.destroyFence(syncFence[i]);
	}
