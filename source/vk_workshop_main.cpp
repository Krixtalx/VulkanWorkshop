...
int main()
{
	...
	for (size_t currentOffset = 0; currentOffset < CONCURRENT_FRAMES; ++currentOffset) {
		clearBuffers[currentOffset] = device.createBuffer(createInfo);
		if (currentOffset == 0) {
			// Allocate the memory (we want host-coherent memory):
			memory = helpers::allocate_host_coherent_memory_for_given_requirements(physicalDevice, device, createInfo.size * CONCURRENT_FRAMES, device.getBufferMemoryRequirements(clearBuffers[currentOffset]));
		}
		...
		memcpy(clearColorMappedMemory, (*clearColorData)[currentOffset].data(), createInfo.size);
		device.unmapMemory(memory);

		// Make sure to clean up at the end of the application:
		cleanupHandlers.emplace_back([device, buffer = clearBuffers[currentOffset], memory]() {
